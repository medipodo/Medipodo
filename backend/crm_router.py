...
@router.delete("/assessment-requests/{record_id}", status_code=204)
async def crm_delete_request(record_id: str):
    """Permanently delete an assessment row and all attached storage images."""
    try:
        deleted = delete_assessment(record_id)
    except Exception as exc:  # noqa: BLE001
        logger.exception("CRM delete failed")
        raise HTTPException(status_code=502, detail="Kayıt silinemedi.") from exc
    if not deleted:
        raise HTTPException(status_code=404, detail="Kayıt bulunamadı.")
    return None
...
async def crm_health():
    """Lightweight health snapshot used by the CRM Settings modal."""
    supabase_ok = ping_supabase()
    return {
        "crm_version": CRM_VERSION,
        "backend": "ok",
        "supabase": "ok" if supabase_ok else "error",
        # The backend is hosted on Render — if this endpoint responded, Render
        # is up. Reported as a discrete field so the UI can show it explicitly.
        "render": "ok",
    }
