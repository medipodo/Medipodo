...
def delete_assessment(record_id: str) -> bool:
    """Permanently delete a row + all its storage images.

    Returns True if the row existed and was deleted, False if not found.
    Storage cleanup failures are logged but do not block the row delete.
    """
    client = get_client()
    # Fetch first so we know which storage objects to remove.
    existing = client.table(TABLE_NAME).select("image_paths").eq("id", record_id).limit(1).execute()
    rows = existing.data or []
    if not rows:
        return False
    paths = rows[0].get("image_paths") or []
    if paths:
        try:
            client.storage.from_(BUCKET_NAME).remove(paths)
        except Exception:  # noqa: BLE001
            logger.exception("Failed to delete storage objects for %s: %s", record_id, paths)
    client.table(TABLE_NAME).delete().eq("id", record_id).execute()
    return True


def ping_supabase() -> bool:
    """Cheap connectivity check used by the CRM health endpoint."""
    try:
        get_client().table(TABLE_NAME).select("id").limit(1).execute()
        return True
    except Exception:  # noqa: BLE001
