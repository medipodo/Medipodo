...
# ---------------------------------------------------------------------------
# Legacy status check store (in-memory) — MongoDB removed; project uses Supabase.
# ---------------------------------------------------------------------------
...
async def create_status_check(payload: StatusCheckCreate):
    ...
    _status_checks.append(doc)
    return obj


@api_router.get("/status", response_model=List[StatusCheck])
async def get_status_checks():
    rows = [dict(r) for r in _status_checks[-1000:]]
    for r in rows:
...
app.include_router(crm_router)  # /api/crm/* — guard at the proxy or in `require_admin`
