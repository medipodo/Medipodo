...
-- ============================================================
-- PERMISSIONS · service_role must be able to read/write the table
-- ------------------------------------------------------------
-- Fixes: "permission denied for table assessment_requests" (SQLSTATE 42501).
-- The service role bypasses RLS but still needs GRANTs when the table
-- was created by another role (e.g. a migration user) or on projects
-- where default privileges were stripped.
-- Safe to re-run.
-- ============================================================

-- 1) Own the table with the standard Supabase superuser role.
alter table public.assessment_requests owner to postgres;

-- 2) Schema usage for the two API roles.
grant usage on schema public to service_role, authenticated, anon;

-- 3) Full privileges to service_role (backend uses SUPABASE_SERVICE_ROLE_KEY).
grant all privileges on table public.assessment_requests to service_role;

-- 4) Any current or future sequences in `public` (covers gen_random_uuid()
--    plus any serial/identity columns added later).
grant all privileges on all sequences in schema public to service_role;

-- 5) Make the grants apply to future objects created in `public`.
...
## 2b. Storage permissions (service_role)

The backend uploads / lists / deletes objects in the `assessment-images` bucket
using the service-role key. Storage RLS lives on `storage.objects`. If your
project was created with the tightest defaults you may also need to grant
service_role explicit privileges on the storage tables. Safe to re-run:

```sql
-- Ensure the bucket exists (private).
insert into storage.buckets (id, name, public)
values ('assessment-images', 'assessment-images', false)
on conflict (id) do nothing;

-- Grant service_role full access to storage metadata + object rows.
-- (service_role already bypasses storage RLS; these GRANTs cover projects
-- where the role lost table-level privileges.)
...
If you hit `permission denied` for `storage.objects`, run the "Storage permissions
(service_role)" SQL block in section **2b** above.

SQL alternative to create the bucket:
```sql
insert into storage.buckets (id, name, public)
values ('assessment-images', 'assessment-images', false)
on conflict (id) do nothing;
